...
wrapFirejail = pkg: pkgs.lib.hiPrio (pkgs.symlinkJoin {
  ...
  postBuild = ''
    ...
    # ==========================================
    # Replace symlinked binaries with custom shell scripts.
    if [ -d "$out/bin" ]; then
      for f in "$out/bin/"*; do
        ...
        # Note: Execution paths are wrapped in double-quotes ("...") to 
        # safely handle any spaces in the filenames, preventing shell expansion errors.
        cat <<EOF > "$f"
#!/bin/sh
...
EOF
    ...
    if [ -d "$out/share/applications" ]; then
      for desktop in "$out/share/applications/"*.desktop; do
        ...
        # OPTIMIZATION: Only rewrite the file if it actually contains an absolute Nix path.
        # This saves disk I/O and speeds up the build by skipping files that don't need patching.
        if grep -qE '^(Exec|TryExec)=/nix/store' "$desktop"; then
          # Strip the absolute path. 
          # Example: 'Exec=/nix/store/123-app/bin/app %u' -> 'Exec=app %u'
          # We use '@' as the sed delimiter to avoid conflicts with regex '|' and path '/'.
...
systemd.user.services.linux-wallpaperengine = {
  ...
  Service = {
    ...
    # PREVIENE LA CREACIÓN DE ARCHIVOS CORE BASURA DURANTE EL APAGADO/REINICIO
    LimitCORE = 0;
  };
};
