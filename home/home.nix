  ...
  postBuild = ''
    ...
    if [ -d "$out/bin" ]; then
      for f in "$out/bin/"*; do
        ...
        # Added quotes around the execution paths to prevent word-splitting
        cat <<EOF > "$f"
#!/bin/sh
...
EOF
    ...
    if [ -d "$out/share/applications" ]; then
      for desktop in "$out/share/applications/"*.desktop; do
        ...
        # OPTIMIZATION: Only perform file I/O if the file actually contains an absolute Nix path
